export default {
  async fetch(req, env) {
    ...
    if (!env.GROUPS_CLAIM_SIGNING_KEY) {
      console.error("[Init] Missing GROUPS_CLAIM_SIGNING_KEY in environment");
    ...
    const isValid = await verifySignature(signatureHeader, rawBody, env.GROUPS_CLAIM_SIGNING_KEY);
    if (!isValid) {
    ...
    // --- Build response ---
    const response = buildGroupsClaimResponse(jsonBody);
