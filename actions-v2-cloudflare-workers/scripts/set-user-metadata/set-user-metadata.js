export default {
  async fetch(req, env) {
    ...
    // --- Build response ---
    const response = addMetadata(jsonBody);
 ...
 */
function addMetadata(jsonBody) {
  // Example last login time as metadata
  const lastLogin = new Date().toISOString();
  const encoded = btoa(unescape(encodeURIComponent(lastLogin)));
  return {
    set_user_metadata: [
      { key: "last-login", value: encoded },
    ],
  };
}
